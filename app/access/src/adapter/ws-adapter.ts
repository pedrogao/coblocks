...
export class WsAdapter extends AbstractWsAdapter {
  ...
  public bindMessageHandlers(
    ...
    const source$ = fromEvent(client, 'message').pipe(
      mergeMap((data) =>
        this.bindMessageHandler(data, handlers, transform).pipe(filter((result) => !isNil(result))),
      ),
      takeUntil(close$),
    );
  ...
  public bindMessageHandler(
    ...
    try {
      ...
      const messageHandler = handlers.find(
        // Attention: dispatch message by `type` property
        (handler) => handler.message === message.type,
      );
      const { callback } = messageHandler;
      return transform(callback(message.data, message.type));
    } catch {
      return EMPTY;
    }
  }
